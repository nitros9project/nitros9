*******************************************************************
* llide_pt - Low-level IDE driver for Pico-Thing
*
* Adapted from llide.asm for the Pico-Thing's PATA interface.
*
* Key differences from standard llide:
*   - Data register (DataReg) is 16-bit: use LDD/STD
*   - No latch register: removed all Latch reads/writes
*   - Byte order: big-endian (D8-D15 at DataReg, D0-D7 at DataReg+1)
*     So: ldd DataReg,x gives A = D8-D15 (high), B = D0-D7 (low)
*
...
* Created (original llide).
*
*     4    2005/12/13  Boisy G. Pitre
* Moved SS.VarSect code into RBSuper for performance
*
*     4r1  2025       Pico-Thing port: 16-bit data register, no latch
...
*         ldb   #6
                    ...
                    pshs      b
                    leay      V.SnsData,u
read@               ldd       DataReg,x read 16-bit word (no latch on Pico-Thing)
                    std       ,y++
                    dec       ,s
                    bne       read@
                    puls      b
...
* Send to data port
                    ...
                    leay      V.ATAPICmd,u
l@                  ldd       ,y++
                    std       DataReg,x write 16-bit word (no latch on Pico-Thing)
                    dec       ,s
                    bne       l@
                    puls      a,y
...
dread@              ldd       DataReg,x read 16-bit word to flush (no latch)
                    lda       Status,x
                    anda      #8
                    bne       dread@
                    bra       initdone
...
* std stores A (high) first so no byte swap needed for geometry fields
...
* discard words 7-48 (42 words); ldd clobbers B so use stack counter
                    ldb       #42
...
* increment physical sector
                    ...
                    bcc       go@
                    inc       V.PhySct,u
go@                 dec       V.SctCnt,u decrement number of hw sectors to write
...
* increment physical sector
                    ...
                    bcc       go@
                    inc       V.PhySct,u
go@                 dec       V.SctCnt,u decrement number of hw sectors to read
                    bne       again@    if not zero, do it again
                    ldd       #NBUSY    /BUSY
                    lbsr      StatusWait wait for proper condition
                    bcs       ex@       branch if error
...
* increment physical sector
                    ...
                    inc       V.PhySct,u
go@
                    dec       V.SctCnt,u decrement number of hw sectors to write
                    bne       loop@     if not zero, do it again
                    clrb
ex@                 puls      x,pc
