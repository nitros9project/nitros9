...
* Debug: print DD.BT (3 bytes) and DD.BSZ (2 bytes) from LSN0
                    ...
                    jsr       <D.BtBug
                    lda       DD.BT,x   DD.BT byte 0 (MSB)
                    lbsr      PrHex
                    lda       DD.BT+1,x DD.BT byte 1
                    lbsr      PrHex
                    lda       DD.BT+2,x DD.BT byte 2 (LSB)
                    lbsr      PrHex
                    lda       #':
                    jsr       <D.BtBug
                    lda       DD.BSZ,x  DD.BSZ high byte
                    lbsr      PrHex
                    lda       DD.BSZ+1,x DD.BSZ low byte
                    lbsr      PrHex
                    lda       #']
                    jsr       <D.BtBug
                    puls      x         restore LSN0 pointer
                  ENDC
...
FragBoot            ldb       DD.BT,x   MSB fd sector location
                  ...
                  IFGT    Level-1
                  IFNE    picothing
                    lda       #'F       FragBoot HWRead returned
                    jsr       <D.BtBug
* Debug: print FD.SIZ (4 bytes) and first segment FDSL.A (3 bytes) + FDSL.B (2 bytes)
                    lda       #'(
                    jsr       <D.BtBug
                    lda       FD.SIZ,x  FD.SIZ byte 0
                    lbsr      PrHex
                    lda       FD.SIZ+1,x FD.SIZ byte 1
                    lbsr      PrHex
                    lda       FD.SIZ+2,x FD.SIZ byte 2
                    lbsr      PrHex
                    lda       FD.SIZ+3,x FD.SIZ byte 3
                    lbsr      PrHex
                    lda       #':
                    jsr       <D.BtBug
                    lda       FD.SEG,x  first segment sector MSB
                    lbsr      PrHex
                    lda       FD.SEG+1,x first segment sector mid
                    lbsr      PrHex
                    lda       FD.SEG+2,x first segment sector LSB
                    lbsr      PrHex
                    lda       #',
                    jsr       <D.BtBug
                    lda       FD.SEG+FDSL.B,x first segment size high
                    lbsr      PrHex
                    lda       FD.SEG+FDSL.B+1,x first segment size low
                    lbsr      PrHex
                    lda       #')
                    jsr       <D.BtBug
                  ENDC
...
GrabBootMem
                  ...
                  IFNE    picothing
                    bcc       btmok@
                    lda       #'X       F$BtMem FAILED
                    jsr       <D.BtBug
                    lbra      error
btmok@              lda       #'A       F$BtMem returned OK
                    jsr       <D.BtBug
                  ENDC
                  ELSE
                    os9       F$SRqMem
...
* the statics pointer in U
                  ...
                  IFNE    picothing
                    ...
                    lbsr      PrHex
                    lda       FDSL.A+1,x segment sector mid
                    lbsr      PrHex
                    lda       FDSL.A+2,x segment sector LSB
                    lbsr      PrHex
                    lda       #',
                    jsr       <D.BtBug
                    lda       FDSL.B,x  segment size high
                    lbsr      PrHex
                    lda       FDSL.B+1,x segment size low
                    lbsr      PrHex
                    lda       #'}
                    jsr       <D.BtBug
                    puls      x
