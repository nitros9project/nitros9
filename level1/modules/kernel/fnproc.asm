...
BackTo1             equ       *
                  IFNE    picothing
* Print dispatch token: 'n' + state-char + 2-hex-task# + 'G' + quick-flag-char
* Example output: n002G1  (state=user, task=02, quick=1)
                    pshs      a,b,x     save registers (X=process descriptor)
                    lda       #'n       print 'n' for next dispatch
                    jsr       <D.BtBug
                    lda       P$State,x get process state byte
                    lsra                shift bit 7 (system state) into bit 0
                    lsra
                    lsra
                    lsra
                    lsra
                    lsra
                    lsra
                    adda      #'0       convert 0/1 to ASCII character
                    jsr       <D.BtBug  print state character
                    ldb       P$Task,x  get task number
                    lbsr      PicoBtHex print task# as 2 hex digits
                    lda       #'G       print 'G' separator
                    jsr       <D.BtBug
                    ldb       <D.Quick  get quick return flag
                    addb      #'0       convert 0/1 to ASCII character
                    tfr       b,a       move to A for D.BtBug
                    jsr       <D.BtBug  print quick flag character
