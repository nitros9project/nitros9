...
**** Requires a new global to keep track of how many keys are down.
                tst       D.WBKKyDn       one or more keys currently down?
                bne       scan@             scan if so
...
* Get character from table
                    ...
                    leax      WBKKeys,pcr point to the non-SHIFT key table
                    lda       D.KySns    get the key sense values
                    bita      #SHIFTBIT  check for SHIFT down
                    beq       noshift@   branch of SHIFT is UP
                    leax      WBKShiftKeys,pcr else point to the SHIFT key table
...
* Key is going UP
keyup@
                    dec      D.WBKKyDn    decrement the key down count
...
* Key is going DOWN
keydown@
                inc       D.WBKKyDn   increment the key down count
                cmpa      #META			is this the META key?
...
* This small table handles the META characters on the K keyboard.
MetaTab             fcb       '7,'~
                    fcb       '8,'`
                    fcb       '9,'|
                    fcb       '0,'\
