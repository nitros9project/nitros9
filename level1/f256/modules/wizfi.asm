...
* F$IRQ packet.
*
WIZFI_INTERRUPT     equ       INT_TIMER_0
IRQ_Pckt            equ       *
...
* Fast
* 25,175,000 / 11520 Bytes Per Second (WizFi is byte-based) = 2185 ticks @ 25.175Mhz (8, 137)
* 25,175,000 / 23040 Bytes Per Second (WizFi is byte-based) = 1092 ticks @ 25.175Mhz (4,  68)
* 25,175,000 / 92160 Bytes Per Second (WizFi is byte-based) =  273 ticks @ 25.175Mhz (1,  17)
*
* rates and results
...
TRATE equ 438
QUO equ TRATE/256
REM equ TRATE-(QUO*256)

                    lda       #REM
                    ldb       #QUO
                    stb       T0_VAL+0            registers are still Little Endian?
                    sta       T0_VAL+1
                    clr       T0_VAL+2
                    lda       #%00000001
                    sta       T0_CTR
...
* Install IRQ servicer based on INT_PENDING_0 register
                    ...
                    leay      ISRoutine,pcr       and the service routine
                    os9       F$IRQ               install the interrupt handler
                    bcc       g@                  branch if success
                    os9       F$PErr
g@                  lda       INT_MASK_0          else get the interrupt mask byte
...
* register is available
        ...
        *             leay      ISRoutine,PCR       get address of interrupt routine 
        *             leax      DMSK,PCR            get VIRQ mask info
        *             os9       F$IRQ               install onto table
        *             lbcs      InitExit            exit on error
        ...
        *             lbcs      InitExit            exit on error
                    ...
                    ldb       #IRQ_State_ListenPacket
                    stb       IRQ_State,u



*     ldd       #$100                          make data size an even number of pages
*     pshs      u
*     os9       F$SRqMem            causes driver to malfunction, why?
...
* Pending changes:
* [done] The outgoing packet builder needs to fetch up to 256 bytes from the TxD circular buffer.
* The incoming packet listener needs to write up to 256 packet bytes into the RxD circular buffer.

ISRoutine           pshs      cc,dp,x
                    ...
                    lbne      ISRExit
                    lbsr      iRxFCheck
                    lbeq      PacketSender         Send pending TxD packets only if no RxD
                    lda       [ind_DataReg,u]
                    sta       LastRxD,u
...
x@                  leax      -1,x
                    stx       IpdLen,u
r@                  ldb       PacketChannel,u
                    cmpb      DeviceChannel,u
                    lbeq      ISRReadExit
                    lbra      ISRExit

ListenPacket        leax      strIPD,pcr          point to start of IPD string constant
                    ldb       PacketReadPos,u           what character position are we at?  +  P  D  ,   ?  0-3
                    cmpb      #4
                    ...
                    lbsr      DecBin
                    std       IpdLen,u
m@                  inc       PacketReadPos,u
x@                  lbra      ISRExit             Exit with no data
strIPD              fcc       "+IPD,$,#####:"
t@                  clr       PacketReadPos,u
                    clr       IRQ_State,u        Switch to data mode for the next cycle
                    bra       x@
mc@                 cmpa      b,x                 Match exact char from RxD FIFO
                    beq       m@
                    clr       PacketReadPos,u           Match failed, quit parsing the IPD and start again
                    bra       x@
ms@                 clr       PacketChannel,u
                    ...
                    clr       IpdLen+1,u
                    bra       m@
PacketSender        inc       WritePacketTimer,u
...
c@                  lda       DeviceChannel,u
                    ...
                    leax      strDecimal5,u
                    tfr       y,d
                    lbsr      Word2Dec3           Make sure to use 3-digit ASCII, not 5-digit yet for this test
                    lda       ,x+
                    lbsr      SendByte
                    lda       ,x+
                    lbsr      SendByte
...
ISRWake             clrb                          clear Carry (for exit) and LSB of process descriptor address
                    lda       V.WAKE,u            anybody waiting? ([D]=process descriptor address)
                    beq       ISRExit             no, go return...
...
ReadSlp
                    ...
                    orb       #Suspend
                    stb       P$State,x
n@                  lbsr      Sleep1              go suspend process...
