...
cont@ lda <M$Opt,y get number of option bytes in descriptor
 ...
 anda #%0001111 keep number of 256 byte buffer pages to use for receive buffer
...
cont2@ stx <rxBuffStartPtr save pointer to start of receive buffer
 stx <rxNextBytePosition ?active start pointer?
 stx <u002E ?active end pointer?
 std <rxBuffSize save size of receive buffer
 leax d,x point to end of receive buffer
 stx <u0030 save end of received buffer pointer
...
L041C orcc #IntMasks else turn IRQs off
 ...
 std <rxNextBytePosition save in static memory
...
IRQSvc clrb
 ...
 ldb UART_FCR,y get interrupt status register
...
UnkIRQ ldb UART_FCR,y get interrupt status register
...
* Entry: Y=pointer to 16550 hardware
RxReadyIRQ  ldx <rxNextBytePosition get pointer to where next received byte will go
...
RxDataTimeoutIRQ ldx <rxNextBytePosition
L04BE lda UART_LSR,y
L04C0 bita #$1E
 beq L04C9
 lbsr L0654
 bra L04BE
L04C9 bita #$01
 beq L04D1
L04CD bsr GetNextRXByte
 bra L04BE
L04D1 tst <sigCode is there a signal code?
...
GetNextRXByte lda UART_TRHB,y get character from data register on 16550
 ...
 tst <u0022 ???
 bne L050E if set, append character to buffer
...
L0500 cmpa <V.XON was it transmit on character?
 beq L0578 yes, go do
...
L0518 cmpx <u002E
 bne L052C
 ldb #%00000010 set 2nd bit in error accumulator
...
L0669 bita #$10
 bne L0673
 orb #%00001000 set bit 3 in error accumulator
 orb <V.ERR merge with current error accumulator
 stb <V.ERR and save it back
L0673 puls pc,b
