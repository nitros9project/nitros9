...
*       SS.SidLoad ($93) - F$Move stream from R$X for R$Y bytes
*                          into our F$SRqMem'd 4 KB buffer
*       SS.SidStart($94) - reset voices to start, LoadState=2
*       SS.SidStop ($95) - silence all voices, LoadState=1
...
StreamLen           rmb       2                   actual stream bytes
V1State             rmb       VSSize
V2State             rmb       VSSize
V3State             rmb       VSSize
VMem                equ       *
...
* SS.SidLoad - F$Move caller(R$X, R$Y) into our buffer.
*   Reject E$DevBsy if playing.
*   Validate count in [9 .. SidBufSize].
*   F$SRqMem on first call.
*   Validate header (3 BE word offsets, monotonic, fit in count).
*   Init per-voice state; LoadState=1 (loaded but stopped).
VSSttLoad           lda       LoadState,u
                    cmpa      #2
                    bne       VSLoad1
                    comb
                    ldb       #E$DevBsy
...
VSLoad1             clr       LoadState,u         invalidate any prior load
                    clr       VoiceMask,u
                    ldx       PD.RGS,y
                    ldd       R$Y,x
                    cmpd      #SidMinStream
...
* First-time allocation.
                    ...
                    std       DataSize,u
                    bra       VSLoadCopy
VSLoadAFail         puls      y,u
                    rts

VSLoadCopy
                    pshs      u                   [,s]=driver static
                    ldx       PD.RGS,y
                    ldd       R$X,x               D = caller src ptr
                    pshs      d                   [,s]=src ptr, [2,s]=static
                    ldd       R$Y,x               D = byte count
                    pshs      d                   [,s]=count, [2,s]=src, [4,s]=static
                    ldx       <D.Proc
                    lda       P$Task,x
                    ldb       <D.SysTsk
                    puls      y                   Y = count
                    puls      x                   X = src
                    ldu       ,s                  U = driver static (peek)
                    ldu       DataPtr,u           U = dest
                    os9       F$Move
                    puls      u                   restore driver static
                    bcc       VSLoadValHdr
                    rts                           F$Move failed: err in B, carry set

VSLoadBadArg        ldb       #E$IllArg
                    comb
                    rts
...
* Init V3
                    ...
                    lda       #$01
                    sta       LoadState,u         loaded, not playing
                    clrb
                    rts

VSLoadBadHdr        ldb       #E$IllArg
                    comb
                    rts

* SS.SidStart - reset voices to start, set master regs, enter play.
