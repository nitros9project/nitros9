...
RaCopyTmpOrig               ldq       1,y                 Get Exponent & 1st 3 bytes of mantissa
...
RaAlignPosExp               leax      ,y                  Point X to temp var
                    bsr       RaShiftMant               Shift mantissa to match other value
                    stq       2,y                 Save shifted result
* Equal exponents come here
RaEqualExp               ldq       8,y                 Get mantissa of dest var into Q
                    tst       1,y                 Check exponent of temp var
                    bne       RaSubMant               <>0, go do Subtract again
RaAddMant               addw      4,y                 32 bit add of Q+[2,y]
                    adcd      2,y
...
RaNormCheck               tsta                          Check sign of MSb of Q
                    bmi       RaNormRound               Set, skip ahead
                    andcc     #^Carry             Force carry bit off (for ROLW since no LSLW)
RaNormLoop               dec       7,y                 Drop exponent of dest var by 1
                    bvc       RaNormStep               Not underflowed, continue
                    puls      x                   Pull X back before zeroing out answer
                    bra       RmulZeroResult               Underflow; answer=0

RaNormStep               rolw                          32 bit multiply by 2
                    rold
                    bpl       RaNormLoop               Keep doing until a set bit comes out
RaNormRound               addw      #1                  Add 1 to Q
                    adcd      #0
                    bcc       RaSaveResult               No carry, skip ahead
                    rora
                    inc       7,y
RaSaveResult               std       8,y                 Save MSW of answer
                    tfr       w,d                 Move LSW to D
                    lsrb                          Eat sign bit
                    lslb
                    orb       ,y                  Put in sign of result
RaSaveSignMant               std       $A,y                Save LSW with sign bit
...
*        <RealShiftCnt = ABS difference of exponents
* Exit:  Q:32 bit shifted mantissa
RaShiftMant               suba      #24                 24-31 bit shift?
                    ...
                    sta       <RealShiftCnt              Save # of shifts required (1-7)
                    ldq       2,x                 Get # to shift
RaShiftLoop2               lsrd                          Shift 32 bit # (worst case is 180 cycles)
                    rorw
                    dec       <RealShiftCnt              Dec # shifts left to do
                    bne       RaShiftLoop2               Keep doing until done
                    rts
